...
async function main() {
    try {
        ...
        if (!adminIdentity) {
            ...
            console.log('Run the registerOrg2Admin.js application before retrying');
            return;
        }
        ...
        // ⚠️  role value is 'insuranceAgent' — must match chaincode checks in issueInsurance & approveClaim
        const secret = await ca.register({
            affiliation: 'org2.department1',
            enrollmentID: 'insuranceAgent-Rama',
            role: 'client',
            attrs: [
                { name: 'role', value: 'insuranceAgent', ecert: true },  // ← FIXED (was 'agent')
        ...
        console.log('Successfully registered and enrolled "insuranceAgent-Rama" and imported it into the wallet');
 
        // --- Onboard the agent on the ledger (calls chaincode) ---
        ...
        console.log("=== Onboard Agent success ===", res.toString());
